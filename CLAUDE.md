...
- **Self-reinforcing cycles**: cycles like log→wood→seeds→seedlings→log are handled by a post-convergence pass that detects export-consuming recipes with massively overproduced outputs (>5x demand), caps them, then re-converges with bidirectional scaling until stable. The cycle contracts to its natural fixed point (e.g., log block exports 1.47/s of the 1.5/s produced, consuming only 0.03/s internally).
