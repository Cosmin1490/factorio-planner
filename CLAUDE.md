...
- **Voiding buildings**: sinkhole (liquids), exhaust pipe (gases), burner (non-fuel solids only — requires fuel like coal to operate, produces ash). **Solid fuels (coal, coke, wood, etc.) cannot be voided directly** — the burner consumes them as fuel, not as waste. To void a solid fuel, convert it to a fluid first (e.g., coal-gas-from-coke: coke → coal-gas + tar), then void the fluids.
