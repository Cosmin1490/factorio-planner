...
5. **Classify before linking** — (a) valuable to another block → export, (b) convertible to something valuable → convert then export, (c) pure waste → void (py-burner for solids, sinkhole for liquids, exhaust for gases). Prefer (a) > (b) > (c). Use overflow-to-void to combine: try to export, void only surplus.
...
- **Voiding buildings**: sinkhole (liquids), exhaust pipe (gases), py-burner (any solid). The py-burner uses `*-pyvoid` recipes (category `py-incineration`, crafting_speed 5) that probabilistically destroy items — typically 80% destroyed per cycle (1 in → 0.2 out). One py-burner handles several items/s. There are 4,500+ pyvoid recipes covering nearly every solid item.
- **Conversion voiding** — when the intermediate has value, convert before voiding to capture that value. But **conversion has a cost**: the conversion recipe speed must keep up with production. Always compute the building ratio before committing. Example: `coke-co2` (5s, 2 coke → 80 CO2) processes only 0.4 coke/s per hpf — voiding 2 coke/s from one `coke-coal` hpf requires 5 conversion hpf (unviable). Compare `coal-gas-from-coke` (5s, 20 coke) at 4 coke/s per distilator — 1 distilator per 2 production hpf (viable). When conversion is too expensive, just pyvoid the solid directly.
