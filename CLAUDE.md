...
- [ ] **Power modeling in solver**: compute total MW per solution automatically (count × energy_usage × 60). Could be a post-solve report or a first-class constraint (cap total power, force solver to prefer efficient recipes). Currently done manually after each solve.
...
- [ ] **Standard module/beacon strategy**: methodology covers bio modules in detail but says nothing about productivity/speed modules and beacons — the single largest late-game efficiency lever. Productivity reduces raw material demand multiplicatively; speed beacons reduce building count. Methodology needs: when to apply productivity (most expensive recipes first), never speed in the machine (only via beacons), how productivity changes solver ratios (output increases, craft time increases). Solver needs to account for productivity bonus on output amounts.
- [ ] **Train station throughput planning**: rule 30 describes station naming/circuits but not capacity. Missing: trains/minute per station (function of route length + signal density), when to add parallel stations, how to validate the train network can deliver inputs at the rates block delta planning (rules 10-13) demands. At 15+ blocks pulling from the same source, single stations become bottlenecks.
- [ ] **Circuit patterns for deadlock prevention**: methodology states "multi-product recipes stall when ANY output backs up" and describes overflow-to-void conceptually, but never specifies the circuit implementation. Add a circuit cookbook section: SR latch hysteresis for pumps, overflow valve wiring, conditional inserter loading for multi-product balancing. Gap between knowing the principle and building it reliably.
- [ ] **Recipe cycle detection in solver**: Pyanodon has genuine circular dependencies (ash loops, coal-gas feedback). Solver handles these implicitly through matrix construction but never detects or warns about cycles. When a cycle causes degenerate scaling, you get astronomical numbers with no explanation. A pre-solve cycle detection pass would make failures diagnosable instead of mysterious.
