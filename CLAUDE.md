...
- **Voiding buildings**: sinkhole (liquids), exhaust pipe (gases), burner (non-fuel solids only — requires fuel like coal to operate, produces ash). **Solid fuels (coal, coke, wood, etc.) cannot be voided directly** — the burner consumes them as fuel, not as waste.
- **Conversion voiding has a cost** — converting a solid fuel to voidable fluids requires buildings, and the conversion recipe speed must keep up with production. Always check the ratio: if the conversion recipe is slow, you may need impractically many buildings. Example: `coke-co2` (5s, 2 coke → 80 CO2) processes only 0.4 coke/s per hpf — voiding 2 coke/s from one `coke-coal` hpf requires 5 conversion hpf, a 5:1 ratio that makes this approach unviable at scale. Compare `coal-gas-from-coke` (5s, 20 coke → coal-gas + tar + ash) at 4 coke/s per distilator — 10x faster, 1 distilator per 2 production hpf. Always compute the conversion building ratio before committing to a voiding strategy.
