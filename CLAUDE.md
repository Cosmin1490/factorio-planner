...
1. **Identify the bottleneck** — most expensive input (deep chain, slow buildings, rare ores, animal husbandry). When the target depends on a key intermediate, search for chains that maximize production of that intermediate — don't just compare recipes that directly produce the target. Example: acetylene needs coke; the tar refining chain yields 6× more coke per raw-coal than coke-coal, making it the better fuel path despite not producing fuel directly.
...
- **Conversion voiding** — when the intermediate has value, convert before voiding to capture that value. But **conversion has a cost**: the conversion recipe speed must keep up with production. Always compute the building ratio before committing. Example: `coke-co2` (5s, 2 coke → 80 CO2) processes only 0.4 coke/s per hpf — voiding 2 coke/s from one `coke-coal` hpf requires 5 conversion hpf (unviable). Compare `coal-gas-from-coke` (5s, 20 coke) at 4 coke/s per distilator — 1 distilator per 2 production hpf (viable). When conversion is too expensive and the item isn't a solid fuel, pyvoid it directly instead.
- **Convert before voiding when the intermediate has value** — stone can be pyvoid'd directly, but converting to saline-water (10 stone + 100 water → 50 water-saline) creates a useful intermediate. Apply the overflow-to-void pattern on the saline-water: export what consumers need, sinkhole the rest. Example: copper block converts stone → saline-water, exports it for EC block, voids only surplus.
...
20. **Design for upgrade, build with what you have** — when higher-tier modules or buildings are unlocked but impractical to bootstrap (e.g., bio mk02 modules with 0.5% drop rate = hours to fill slots), design with the achievable tier. But plan for the upgrade:
    ...
    **(b) Consider building to upgraded ratios now** — when ratios will change on upgrade, it may be better to lay out the block with upgraded-tier building counts today, even though current-tier modules mean underproduction. Short-term output is lower, but when you upgrade modules it's a drop-in swap — zero redesign (design cost) and zero teardown/rebuild (build cost). The tradeoff: accept temporary inefficiency vs. save both costs later.
    
    **(c) Separate design cost from build cost** — redesigning a block (re-deriving ratios, re-running solver, planning layout) costs the same regardless of tech level — it's always human+Claude time. *Building* cost (placing entities, routing belts, tearing down old layouts) decreases with tech: early game is manual placement with slow belts, late game has construction bots and blueprints. Design-for-upgrade avoids paying *both* costs again. This matters most early game when build cost is high; late game, cheap builds make redesign-then-rebuild acceptable, but the design cost never goes away.
