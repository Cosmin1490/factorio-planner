...
4. **Rank by:** efficiency (raw materials/output) > complexity (recipes/buildings) > convenience. Watch for "later game" recipes that exist to consume excess byproducts — traps at early tech. **Always run the numbers before eliminating** — two paths sharing an upstream input can have wildly different per-unit consumption. Don't dismiss on structural similarity alone; quantify first.
    ...
    **Trace "simple" alternatives to their full ongoing costs.** A recipe with fewer dependencies can look cheaper until you quantify the consequences: higher ore consumption means more mining drills and faster deposit depletion, worse ore:product ratios mean more byproduct waste to manage (and some byproducts like stone have no clean void path), and higher fuel consumption is a permanent per-second cost that never amortizes. Compare this against the "complex" alternative's upfront infrastructure: if that infrastructure serves multiple future consumers, its cost amortizes across the factory while the "simple" path's ongoing costs compound with every stamp. Example: low-grade iron smelting (3:1 ratio, no dependencies) vs BOF casting (1.4:1, needs oxygen + borax blocks). Low-grade looks simpler but requires 50/s ore (vs 9/s), produces 10/s stone with no pyvoid recipe, and burns 90/s acetylene. The oxygen block serves future antimony/fish/aromatics; the borax block is already needed. Three blocks in sequence vs one "simple" block with permanent waste — the infrastructure path wins.
