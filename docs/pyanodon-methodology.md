...
- **Voiding buildings**: sinkhole (liquids), exhaust pipe (gases), py-burner (non-fuel solids). The py-burner is a furnace that requires solid fuel (accepts chemical, biomass, jerry, and nuke categories) and uses `*-pyvoid` recipes (category `py-incineration`, crafting_speed 5) to probabilistically destroy items — typically 80% destroyed per cycle (1 in -> 0.2 out at probability). One py-burner handles several items/s. **Solid fuels (coal, coke, raw-coal, wood) have NO pyvoid recipes and cannot be destroyed** — they are consumed as fuel by the burner, producing `burnt_result` (ash for coal/coke/raw-coal). To void a solid fuel, convert it to fluids first (e.g., `coal-gas-from-coke`: coke -> coal-gas + tar + ash), then void the fluids and pyvoid the ash. **Note:** `recipes --consumes` does not show pyvoid recipes (filtered category). Use prototype data to verify: `data.recipes["<item>-pyvoid"]`. **Py-burner fuel cost is usually negligible.** Compute `energy_usage / fuel_value` — at 180 kW with coke (5 MJ), that's 0.036/s. Check fuel sources in order: (1) block produces it internally (divert — nearly free), (2) block already imports it for another purpose (share — free), (3) new import required (one station for minimal traffic). At typical rates, even option 3 is cheaper than building conversion infrastructure. Compare this cost against conversion (rule 5c) before defaulting to a washer or other conversion chain.
...
- **Overflow-to-void pattern**: don't void blindly — prioritize real consumption, only void surplus. Wire a pump or overflow valve with circuit condition: fill the load station buffer first, overflow excess to a sinkhole/exhaust. When a consumer eventually connects, trains pull from the station and less goes to void automatically. No block redesign needed. Example: tar refinery produces 280/s pitch with no current consumer — overflow valve after the pitch tank routes excess to sinkhole, block keeps running for creosote/gasoline/coke/middle-oil. **Prefer voiding at source.** Void inside the producing block when possible — it's simpler and avoids train scheduling complexity. When voiding doesn't fit in-block (e.g., solid fuel needing fluid conversion), a single dedicated void destination at the lowest train priority is acceptable. Never use more than one void destination per item — multiple void blocks compete with real consumers for train deliveries and priorities can't distinguish them.
...
- **Pyanodon hard mode (`pyhardmode` mod) restricts voiding.** Sinkholes are fixed to the `water-pyvoid-fluid` recipe (base water only — not water-saline or other variants) and must be placed on lakes/rivers. Gas vents require filtration media — venting has an ongoing material cost. Py-burners don't destroy items; they generate heat, which must be consumed (incubators, heat exchangers, etc.). Category (d) "pure waste → void" from rule 5 effectively disappears: every byproduct must be (a) recycled, (b) exported, or (c) converted. Fluid byproducts that would normally be vented (e.g., hydrogen from electrolysis) must be burned for power with guaranteed consumption — which requires electrical isolation of the burn loop (dedicated mini-network for the boiler + engine section with its own consumers) since grid-wide steam engines throttle proportionally to oversupply. Plan for hard mode constraints at block design time, not as a retrofit.
...
11. **Compute block delta from inventory** — compare consolidated demand against what's already built. The **block delta** is the set of new blocks needed to close the gap.
    ...
    Steps: (1) read the block inventory — what's built, what rate, what it exports. (1b) **Verify units match** — if demand is in item X but inventory produces item Y, find the conversion recipe and convert before comparing. Example: demand 9.4 wood/s vs inventory 1.47 log/s — `log-wood-fast` converts 4 log → 40 wood (10 wood/log), so actual supply is 14.7 wood/s (surplus, not a deficit). Skipping this step produces false shortfalls. (1c) **Use solver-validated rates** — compare solver import rates from rule 10 step 6 against inventory, never the hand-traced estimates. (2) For each demanded item: if existing supply >= demand, no new block needed; if not, compute shortfall. (3) Classify each shortfall by volume: **bus-scale** (>~0.5/s or Tier A/B consumers) -> dedicated block; **niche** (<~0.5/s, Tier C/D consumers) -> inline into consumer block. (4) Check mining requirements (rule 22) for any new ore — mining inputs may surface additional blocks or bus fluids. (5) Identify **bus fluids** — fluids that multiple new blocks import (e.g., acetylene for lead + titanium mining). These need their own export infrastructure and create ordering dependencies. (6) **Forward-trace surpluses against deficits** — for each voided or surplus output in the block inventory, run `recipe-tree --produces-from <item> --unlocked --depth 3` and check if the output reaches any item on the deficit list. When multiple surpluses from the same block converge into one deficit item, that's a strong signal for a conversion block that captures waste value AND closes a deficit simultaneously. This is the supply-side complement to rule 10's demand-side backward trace — backward tracing asks "what do I need?", forward tracing asks "what can my waste become?" Example: tar refinery voids pitch (280/s) and coke (15/s). Backward tracing from logistic science finds acetylene as a deficit (mining fluid for lead/zinc/titanium). Forward tracing from pitch finds pitch-refining → more coke; forward tracing from coke finds lime → calcium-carbide → acetylene. Both waste streams converge into the same deficit item. An acetylene block consumes both wastes, unstalls the tar refinery, and supplies a bus fluid — triple value vs. voiding each independently.
...
16. **Consumer count** — items consumed by many recipes are natural bus items. Count all recipes (not just currently unlocked) so boundaries hold as you research more techs (via `recipes --consumes <item>`):
    ...
    **Consumer count is inflated by mall recipes.** Most bus items are heavily consumed by mall recipes (buildings, vehicles, infrastructure) which inflate the count. The tier classification still works — even the production minority justifies Tier A blocks — but high consumer count alone can tempt you into building dedicated blocks for items that are cheap to inline. A MAM mall collapses all mall demand for `crafting`-category items into a single low-throughput consumer. Items requiring specialized factories can't benefit from this and follow standard boundary rules.
...
19. **Stable physical properties** — good boundaries have uniform properties across consumers. Iron-plate at X/s is iron-plate regardless of consumer. Temperature-variant fluids (coke-oven-gas at 100C vs 250C) are poor boundaries unless the solver's temperature-linked columns handle them — unconstrained fluids still share one column, verify manually. Steam specifically is never trained (rule 9).
    ...
    **Same fluid in and out ≠ net zero.** When a recipe lists the same fluid as both ingredient and product (e.g., warm-stone-brick-1: 100 COG in → 100 COG out), check the output temperature — it may be degraded. COG enters at 250°C and exits at 100°C; the heat is consumed, and the cold output cannot recirculate. Treat temperature-degraded fluids as consumed inputs and cold outputs as byproducts. The solver handles this correctly via temperature-linked columns, but manual analysis and block design must account for the ongoing hot-fluid import cost.
    
    **Fluid boundaries are cheaper to transport than solid boundaries.** Pumps move ~12,000 fluid/s vs inserters at ~45 items/s on a blue belt — a ~250x throughput ratio. Fluid infrastructure (pipe + pump) is also simpler and cheaper to build than solid infrastructure (belt + splitter + inserter + underground belt). This means: (a) when choosing between a fluid and solid form of the same resource as a boundary, prefer the fluid form; (b) fluid imports/exports cost less station throughput capacity than solid ones — a single fluid wagon fills/drains faster than a cargo wagon; (c) don't penalize fluid boundaries for "being fluids" during boundary selection — the logistics are easier, not harder.
...
20. **City block space budget** — in train-based city block architectures, each block has finite space split between factories and train stations (1 station per item, input or output). Three tools to fit a sub-factory into a block:
    ...
    **Switchable recycling: circuit bypass for MW/import trade-off.** When recycling adds significant power cost (electrolyzers, chemical plants), build the full recycling setup but wire a bypass that can disable recycling, routing the byproduct to overflow-to-void instead. Two operating modes: recycling ON (higher MW, lower imports — e.g., 54 MW / 75/s water) vs recycling OFF (lower MW, higher imports — e.g., 4 MW / 225/s water). The switch manages power budget explicitly — flip it when power is tight, flip back when train throughput is the bottleneck. The bypass costs almost nothing to build (the overflow-to-void sinkhole already exists per rule 5), and the block naturally migrates toward always-recycle as train traffic grows and power scales. Size the import station for the non-recycling rate so bypass mode works without station changes. Implementation options — pick one per block based on what's being recycled:
    - **Power switch** (best for fluid recycling with electric buildings): wire a power switch on the recycling sub-grid. OFF = electrolyzers lose power, stop processing, byproduct overflows to sinkhole naturally. Cleanest — the switch literally is the MW trade-off, no pipe/belt wiring needed.
    - **Pump with circuit condition** (fluid recycling, finer control): pump on the byproduct pipe enabled by a signal. OFF = pump stops, fluid overflows. Allows partial recycling or signal-driven automation (e.g., enable recycling when power surplus signal > threshold).
    - **Belt circuit condition** (solid byproduct recycling): belt segment with circuit enable/disable routes items to recycling buildings or directly to pyvoid. Same logic, different transport.
...
21. **Single-item smelting** — at bus-scale volumes, each plate gets its own city block. Prefer steel-furnace (2x2, speed 4, fluid-burning) over advanced-foundry (6x6, speed 1, electric) — 33x more plates per tile. The solver can use advanced-foundry for simplicity (see checklist); real builds use steel-furnace for density. Steel-furnace burns any fluid fuel — consumption rate scales inversely with fuel value: `fuel_rate = 6 MW / fuel_value`. **Solver limitation:** steel-furnace has `fluid_energy_source`, which the solver does not model — fuel consumption is silently ignored. Compute fuel needs manually using the formula above. The solver correctly reports building counts and recipe rates; only fuel is missing. Higher-value fuels (gasoline 1.2 MJ, COG 1.0 MJ) need less throughput; low-value fuels (coal-gas 0.2 MJ) need 5x more, which has real infrastructure impact (pipe capacity, train trips, station sizing). Choose fuel based on both availability and throughput cost.
    ...
    **On-site vs centralized — stack size is the real driver.** The ore:plate ratio matters because of **train capacity**: raw ores typically have stack size 50, while plates and processed ores have stack size 100 — so raw ore carries half as many items per wagon. The decision tree:
    ...
    1. **Check stack sizes at each processing stage.** If an intermediate (e.g., processed-iron-ore, stack 100) matches plate density, you can stop processing there and train the intermediate — you've already captured the train throughput benefit without needing to bring extra imports to the mine.
    2. **Crush at the mine, smelt centrally** is often the sweet spot. Iron example: raw ore (stack 50) → crush on-site → processed ore (stack 100) → train to centralized smelter. Same train density as plates, but smelting stays near shared infrastructure (borax, oxygen). Always better than training raw ore when an intermediate has a higher stack size.
    3. **Check the conversion ratio at the smelting stage.** Even when processed ore and plates share the same stack size, smelting can amplify volume — e.g., 9/s processed-iron-ore → 10.8/s iron-plate (1:1.2) or 13.5/s with hot-air (1:1.5). This doesn't change stack density, but it means more outbound train trips for plates than inbound for processed ore.
...
23. **Design for upgrade, build with what you have** — when higher-tier modules/buildings are unlocked but impractical to bootstrap (e.g., bio mk02 at 0.5% drop rate), design with the achievable tier but plan for the upgrade. Check **ratio stability**: (1) all buildings have matching tier upgrades -> ratios hold, just need more I/O; (2) only some upgrade -> ratios break, needs redesign; (3) no matching tier downstream -> bottleneck just moves. When ratios will break, consider **building to upgraded ratios now** — accept underproduction today for a drop-in module swap later with zero redesign.
    ...
    **Recipe-variant staging:** some recipes form upgrade pairs — identical sub-components, same building, same craft time, differing only in one input and output multiplier (e.g., `electronic-circuit` uses battery-mk00 → 3 output; `electronic-circuit-2` uses battery-mk01 → 5 output). When this pattern exists, the two recipes aren't mutually exclusive — they're sequential phases. If the higher-tier input's upstream is already available (existing blocks, reasonable build cost), build the best recipe outright. But when it requires expensive new infrastructure (bio chains, deep chemistry, multiple undesigned blocks), consider starting with the lower-tier variant and retrofitting later. Size shared sub-component production for the lower-tier rate (higher per-output-unit, since fewer outputs per craft). When the upgrade input becomes available: swap the one input, change the recipe — shared infrastructure stays, and the oversized sub-components give free bonus capacity. Check for this pattern whenever rule 4 (comparing alternatives) surfaces two recipes that share most ingredients.
...
26. **Block capacity heuristic (bio farms)** — building tile footprint is the dominant factor for bio farm blocks. All bio buildings hit effective speed 1.0 with full modules, so per-farm output is recipe-determined, but how many fit is purely tile footprint vs block area. Actual block size is **~128x128 tiles** (16,384 tile^2), but rail perimeter, stations, belt bus, and pipe routing consume significant space — usable interior is smaller. Empirical data:
    ...
    Small buildings (6x6) leave most of the block free — enough to inline supporting recipes (CO2 production, etc.). Large buildings (13x13) fill most of the usable interior, leaving room only for stations and logistics. When a block is space-constrained, stamp a second copy (rule 25) rather than trying to squeeze more in.
...
**Mall blocks** (infrastructure crafting — trains, belts, inserters, poles, pipes, combinators) are exempt from the production block rules above. Throughput is irrelevant, stamping doesn't apply, belt coupling and bots between subblocks are fine, and the setup can span multiple city blocks. Blocks that primarily feed the mall (as opposed to science or other production) can also cut corners — mall demand is low and bursty, so a lazy block at minimal throughput is fine. Don't optimize ratios or stamp for capacity when the mall is the main consumer. Design a mall block with ingredient groupings (shared inputs minimize station count) and MAM recipe swapping for low-volume items.

**Mall leeches off science — and that's fine.** The mall and science blocks share the same bus. When you build locomotives or stamp rail, bus items flow to the mall instead of science. Science dips temporarily while mall stock replenishes. This is an acceptable tradeoff — reduced science for a few minutes buys infrastructure you need to keep expanding. Prioritize mall over science; the mall produces the trains and belts that let you stamp more science blocks.
...
- **Exclude byproducts that drive scaling** — `--constraint "recipe:product:exclude"` for every byproduct that could cascade. Excludes reduce degrees of freedom, helping the LP find better solutions faster.
...
- **Logistic science pack**: 11,506 → 163 buildings. Progression: 11,506 (naive) → 110 (commodity boundaries + bio modules) → 326 (temperature-linked fluid correction) → 163 (LP cost minimization). Split by shared system (7 pipelines) not by product (4).
