...
**Consumer count is inflated by mall recipes.** Most bus items are heavily consumed by mall recipes (buildings, vehicles, infrastructure) which inflate the count. The tier classification still works — even the production minority justifies Tier A blocks — but high consumer count alone can tempt you into building dedicated blocks for items that are cheap to inline. A MAM mall collapses all mall demand for `crafting`-category items into a single low-throughput consumer (see `data/designs/mall-block.md`). Items requiring specialized factories can't benefit from this and follow standard boundary rules.

**Inline vs dedicated is a spectrum, not a binary.** Some items are always inlined (vacuum — zero inputs, 1 pump, never worth a station). Some are always dedicated blocks (iron-plate — bus-scale, dozens of production consumers). Most live in between: inlined in some blocks, imported in others, or inlined now and extracted to a dedicated block later as more consumers appear. Inlining has a multiplication cost — each block duplicates the setup independently. If you find the same inline appearing in block after block, that's a smell: a dedicated block with one train station serving all of them might be cheaper. But mixed approaches (inline in the mall, import in a high-throughput production block) are perfectly valid. Revisit inline-vs-dedicated as the factory grows.
