...
16. **Consumer count** — items consumed by many recipes are natural bus items. Count all recipes (not just currently unlocked) so boundaries hold as you research more techs (via `recipes --consumes <item>`):
    ...
    **Consumer count is inflated by mall recipes.** Most bus items are heavily consumed by mall recipes (buildings, vehicles, infrastructure) which inflate the count. The tier classification still works — even the production minority justifies Tier A blocks — but high consumer count alone can tempt you into building dedicated blocks for items that are cheap to inline. A MAM mall collapses all mall demand for `crafting`-category items into a single low-throughput consumer (see `data/designs/06-mall-block.md`). Items requiring specialized factories can't benefit from this and follow standard boundary rules.
