...
19. **Single-item smelting** — at bus-scale volumes, each plate gets its own city block. Prefer steel-furnace (2x2, speed 4, fluid-burning) over advanced-foundry (6x6, speed 1, electric) — 33x more plates per tile. Steel-furnace burns any fluid fuel — consumption rate scales inversely with fuel value: `fuel_rate = 6 MW / fuel_value`. Higher-value fuels (gasoline 1.2 MJ, COG 1.0 MJ) need less throughput; low-value fuels (coal-gas 0.2 MJ) need 5x more, which has real infrastructure impact (pipe capacity, train trips, station sizing). Choose fuel based on both availability and throughput cost.
    ...
    **Upgrade path:** see rule 21. Start with the simpler chain to get plates flowing (e.g., stone-furnace before steel-furnace); swap internals when better inputs are available — station layout stays the same.

20. **Ore sourcing: check mining fluid requirements** — many ores require a specific fluid to mine (e.g., acetylene, steam, syngas, aromatics). Basic electric/burner miners only work on `basic-solid` resources (iron, copper, coal, stone). Other ores need fluid-drills, dedicated miners, or ground-borers — each with their own fluid input. Check the resource's `required_fluid` in prototype data (or `mineable_properties` on the resource entity) when planning a mining block.
...
25. **Fluid transport threshold** — one fluid wagon per minute defines the practical throughput ceiling for training fluids:
...
26. **Block design patterns** — all city blocks follow a standard template:
