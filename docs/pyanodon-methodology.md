...
- **Entity names: base tier has no `-mk01` suffix** — `distilator`, `tar-processing-unit`, `automated-factory`, `glassworks` etc. Only higher tiers use `-mk02`/`-mk03`/`-mk04`. The solver errors with "Factory not found" on wrong names. When in doubt, check `data.entities` keys. Exception: some base entities DO use `-mk01` (e.g., `biofactory-mk01`, `micro-mine-mk01`) — there's no universal rule, verify each one.
