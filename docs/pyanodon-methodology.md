...
20. **Stable physical properties** — good boundaries have uniform properties across consumers. Iron-plate at X/s is iron-plate regardless of consumer. Temperature-variant fluids (coke-oven-gas at 100C vs 250C) are poor boundaries unless the solver's temperature-linked columns handle them — unconstrained fluids still share one column, verify manually. Steam specifically is never trained (rule 9).
